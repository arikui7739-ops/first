...
Sub OptimizeABFormationFlow()
    ...
    Call LoadExclusionSettings(dictExcludedMach, excludedLocMach, excludedLocDanFrom, excludedLocDanTo, excludedLocColFrom, excludedLocColTo, excludedLocCount, dictExcludedItemCode, ratioSheetName, maxSwapRows, maxMachNum, abSlotCount, dictTargetRatio)
...
Sub EnsureOperationPanelSheet()
    ...
    wsPanel.Range("B4").Value = _
        ...
        "①VBEでこの4つのモジュール(Module3_石狩・Module8_石狩・Module9_石狩・Module10_石狩、またはModule3～10)を" & _
        "「ファイルのインポート」で追加する" & vbCrLf & _
        "②いずれかのマクロを一度実行する(右の「AB編成動線最適化を実行」ボタンでよい。ファイル選択はキャンセルして" & _
        "かまわない)。これで本シートと「設定」シートが自動作成され、4つのボタンすべてが使えるようになる" & vbCrLf & _
        ...
        "④「ロケ変指示を作成」でロケ変指示を作成(予測データの取込と、「設定」シートの■機番別目標構成比の入力が必要)" & vbCrLf & vbCrLf & _
        "【カスタマイズ】" & vbCrLf & _
        "除外機番・除外ロケーション・除外品コード・機番回数比シート名・入替候補件数・ロケ変候補件数・最大機番・" & _
        "AB間口数・機番別目標構成比などは「設定」シートで変更できます(シートが無ければ実行時に自動作成されます)。" & _
        "機番別目標構成比を入力すると、入替提案が奇数偶数バランスより目標比率への近さを優先します。"
