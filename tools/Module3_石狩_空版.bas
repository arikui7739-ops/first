...
Sub EnsureOperationPanelSheet()
    ...
    ' 以前のバージョンで結合されたセルが残っていると、形の違う範囲を結合しようとしたときに
    ' うまく反映されないことがあるため、先に結合を解除してからやり直す
    ...
    panelDesc = _
        "このワークブックには、AB(自動倉庫ラック)編成の動線最適化に関する6つのマクロが入っています。" & _
        ...
        "範囲が異なる別エリア向けです)。" & _
        "⑥ゾーンバランス(Module10):予測データ・「品名実績」を元に、出荷回数の順位に応じてAB(1～46号機)・" & _
        "Cバラ(51～68号機)・X拡張(70号機以上)の3ゾーン間で入替候補を作成します(段は問いません)。" & vbCrLf & vbCrLf
    panelDesc = panelDesc & _
        ...
        "かまわない)。これで本シートと「設定」シートが自動作成され、6つのボタンすべてが使えるようになる" & vbCrLf & _
    ...
    panelDesc = panelDesc & _
        ...
        "機番別目標構成比を入力すると、入替提案が奇数偶数バランスより目標比率への近さを優先します" & _
        "(ゾーンバランスは除外設定のみ共有し、機番別目標構成比は使いません)。「同時ピッキング改善指示」実行時の" & _
        "実績は「KPI」シートの平均無駄歩行スコア・交換候補件数として記録されます。「KPI」シートの各指標は、" & _
        "判定基準(合格/注意/不合格)に応じて自動的に緑・黄・赤に色分けされます(基準は「KPI」シート右側に一覧表示)。" & _
        "シートタブの並び順がバラバラになったときは「シート並び替え」ボタンで整えられます。"
...
Sub EnsureKPISheet()
    ...
    Call ApplyKPIJudgeCriteria(wsKPI, True)
End Sub
...
Sub EnsureExclusionSettingsSheet()
    ...
    wsSet.Range("A1").Value = _
        ...
        "ゾーンバランス(Module10)で共通して使う設定です(同時ピッキング改善指示(Module1)は別に51～58号機・" & _
        "61～68号機で固定動作します)。" & _
        ...
        "出荷回数のランキングに使う基準はK14/L14の■ゾーンバランス確認基準で変更できます(「品名実績」の曜日、または" & _
        "「予測」=予測データの投入回数_予測をそのまま使う。既定は月曜)。" & _
        ...
        "各表の5行目以降に追加・削除して使ってください。" & _
        "⑦KPI判定基準:「KPI」シートに記録される各指標(AB実績回数比率・AB同時ピッキング回避スコア・均衡化スコア・" & _
        "同号機分散均衡化スコア・平均無駄歩行スコア)は、あらかじめ決められた基準に応じて緑(合格)・黄(注意)・" & _
