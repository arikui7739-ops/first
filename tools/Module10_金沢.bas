...
Sub CreateRelocationPlan()
    ...
    ' 「号機別目標構成比」に数値号機キーが1つも無ければ、近づけるべき目標が無いため号機間バランスを作成できない
    ...
    If Not hasAnyTarget Then
        MsgBox "「設定」シートの「■号機別目標構成比」に号機別の目標値が入力されていないため、号機間バランスを作成できません。", vbExclamation
        Exit Sub
    End If
    ...
    If targetRatioSum <= 0 Or targetedHitTotal <= 0 Then
        MsgBox "目標構成比が設定されている号機の実績(予測)データが見つからなかったため、号機間バランスを作成できません。", vbExclamation
        Exit Sub
    End If
...
' ----------------------------------------------------
' AB(1～24号機)・Cバラ(61～66号機・81～88号機)・拡張(それ以外)の3ゾーン間で、
...
Sub CreateZoneRebalancePlan()
    ...
    Const ZONE_AB_MAX As Long = 24
    Const ZONE_C1_MIN As Long = 61  ' Cバラ01
    Const ZONE_C1_MAX As Long = 66
    Const ZONE_C2_MIN As Long = 81  ' Cバラ02
    Const ZONE_C2_MAX As Long = 88
    ...
    For i = 1 To n
        If IsNumeric(dataArr(i, machColIdx)) And IsNumeric(dataArr(i, danColIdx)) And IsNumeric(dataArr(i, colColIdx)) Then
            ...
            ' 金沢の実機のCバラはC01(61～66号機)・C02(81～88号機)の2ブロック。
            ' それ以外(93～99号機の拡張エリアを含む)はAB・Cバラいずれでもないため拡張(X)として扱う
    ...
    ' ABとCバラの入替のみを対象とする(拡張(X、93～99号機など)は入替元・入替先にしない)
    ...
    wsOut.Range("A2").Value = "AB(1～24号機)"
    wsOut.Range("A3").Value = "Cバラ(61～66号機・81～88号機)"
