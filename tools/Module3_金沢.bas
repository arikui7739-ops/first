...
Sub OptimizeABFormationFlow()
    ...
    ' ABブロック:AB編成のゾーン対象とする号機範囲(複数ブロック可)。既定は金沢の実際のラック配置(1～24の1ブロック)
    ' 61～66(Cバラ01)、81～88(Cバラ02)、その他(拡張X)はAB編成のゾーン・スワップ対象外
    ...
    For Each formationIter In g_CachedFormations
        ...
        For Each recIter In formationIter
            ...
            If IsInABBlock(mach, abBlockFrom, abBlockTo, abBlockCount) Then
                zoneNum = ComputeZoneForMach(mach, abBlockFrom, abBlockTo, abBlockCount) ' 1～24番機は1&2→1,3&4→2…23&24→12
    ...
    For r = 1 To pCnt
        ...
        If Not dictSwapped.Exists(aItem) And Not dictSwapped.Exists(mItem) Then
            ...
            If targetItem <> "" Then
                ...
                ' targetedHitTotalは目標が設定されている号機だけの合計なので、対象号機が
...
' 実績比率・目標比率とも「目標が設定されている号機だけ」の合計(targetedHitTotal/targetRatioSum)で
...
Sub EnsureOperationPanelSheet()
    ...
    panelDesc = _
        ...
        "①AB対面分散(Module3):ピッキング実績ログを解析し、同一号機・同一ゾーン(対面)で同時に出庫されやすい" & _
        ...
        "作成します。実績側は「日別実績」「品名実績」の履歴も自動更新します。" & _
    ...
    panelDesc = panelDesc & _
        ...
        "②「AB対面分散を実行」でピッキング実績ファイル(S71)を解析(品名マスタ・ロケーションマスタは任意)" & vbCrLf
    panelDesc = panelDesc & _
...
Sub EnsureExclusionSettingsSheet()
    ...
    wsSet.Range("A1").Value = _
        "この「設定」シートは、AB対面分散(Module3)・構成比グラフ(Module9)・号機間バランス(Module10)・" & _
        "ゾーンバランス(Module10)で共通して使う設定です。" & _
        "①除外号機:スワップ対象・AB稼働率スコアから号機ごと除外。" & _
        "②除外ロケーション:常時使用スロットなど号機×段×列の範囲を、スワップ対象・稼働率・ヒートマップ集計のすべてから除外" & _
        "(段From/To・列From/Toはそれぞれ空欄にすると「全段」「全列」扱いになる)。" & _
        "③除外品コード:その品コードを格納場所を問わず全ての集計・スワップ対象から除外(CFシートの品コード列と同じ値で指定)。" & _
        "④シート名設定:号機回数比シート名(L4)のほか、入替候補件数(L5、AB対面分散の出力件数)・AB間口数(L6)・" & _
        "ロケ変候補件数(L7、号機間バランスの出力件数。入替候補件数とは別設定)を数値で指定する。" & _
        "⑤ABブロック:AB編成のゾーン・スワップ対象とする号機範囲(複数ブロック可、各ブロック内で号機2台ずつを1ゾーンとして" & _
        "連番付けする)。ブロック外の号機(金沢ではCバラ01=61～66、Cバラ02=81～88、拡張X=それ以外)はAB編成の対象外" & _
        "(ゾーンバランスが対象とするAB・Cバラ・X拡張の範囲とは別の区分です)。" & _
        "⑥号機別目標構成比:各号機の目標構成比(%)を入力すると、AB対面分散の入替提案が奇数偶数バランスより" & _
...
' ブロックをまたぐごとにゾーン番号を連番で継続する(例:金沢は1～24の1ブロックのみなので
' 1&2→1,3&4→2…23&24→12の12ゾーン)。ブロック外の号機は0(対象外)を返す
