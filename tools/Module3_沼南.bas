...
Sub LoadItemAttributeMasterIfSelected(dictItemCategory As Object, dictItemWeightMaster As Object, dictItemVolumeMaster As Object)
    ' 「設定」シートのチェックボックス(L12)がオフなら、ファイル選択ダイアログ自体を出さずに終える
    ...
    ' 「カテゴリー粒度」(L11)に応じて、WF021L1形式CSVの何列目をカテゴリーコードとして使うかを決める
...
Sub EnsureExclusionSettingsSheet()
    ...
    wsSet.Range("L11").Value = "大分類" ' 商品属性マスタのカテゴリー一致判定に使う粒度(大分類/中分類/小分類)
    With wsSet.Range("L11").Validation
        .Delete
        .Add Type:=xlValidateList, AlertStyle:=xlValidAlertStop, Formula1:="大分類,中分類,小分類"
    End With

    wsSet.Range("K12").Value = "商品属性マスタ"
    wsSet.Range("K12").Font.Bold = True
    Dim chkAttr As CheckBox
    Set chkAttr = wsSet.CheckBoxes.Add(wsSet.Range("L12").Left, wsSet.Range("L12").Top - 2, 190, 18)
