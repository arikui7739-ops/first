...
Sub EnsureExclusionSettingsSheet()
    ...
    wsSet.Range("A1").Value = "この「設定」シートは、AB編成動線最適化(Module3)・構成比グラフ(Module9)・ロケ変指示(Module10)で共通して使う設定です。①除外号機:スワップ対象・AB稼働率スコアから号機ごと除外。②除外ロケーション:常時使用スロットなど号機×段×列の範囲を、スワップ対象・稼働率・ヒートマップ集計のすべてから除外(段From/To・列From/Toはそれぞれ空欄にすると「全段」「全列」扱いになる)。③除外品コード:その品コードを格納場所を問わず全ての集計・スワップ対象から除外(CFシートの品コード列と同じ値で指定)。④シート名設定:号機回数比シート名(L4)のほか、入替候補件数(L5、AB編成動線最適化の出力件数)・AB間口数(L6)・ロケ変候補件数(L7、ロケ変指示の出力件数。入替候補件数とは別設定)を数値で指定する。⑤ABブロック:AB編成のゾーン・スワップ対象とする号機範囲(複数ブロック可、各ブロック内で号機2台ずつを1ゾーンとして連番付けする)。ブロック外の号機(沼南ではCバラ01=61～73、Cバラ02=81～93、拡張X=それ以外)はAB編成の対象外。⑥号機別目標構成比:各号機の目標構成比(%)を入力すると、AB編成動線最適化の入替提案が奇数偶数バランスより目標比率への近さを優先するようになり(未入力ならこれまでどおり奇数偶数バランス優先)、ロケ変指示(Module10)はこの目標値が入力されていないと作成できません。合計が100%になっていなくても、入力した号機どうしの相対バランスとして扱われる(Cバラ等AB以外への出荷分があっても問題ない)。各表の5行目以降に追加・削除して使ってください。"
    wsSet.Range("A1").Font.Bold = True
    wsSet.Range("A1").WrapText = True
    wsSet.Range("A1").VerticalAlignment = xlTop
    wsSet.Rows(1).RowHeight = 105
