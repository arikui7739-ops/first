...
Sub EnsureOperationPanelSheet()
    ...
    panelDesc = _
        ...
        "⑤ゾーンバランス(Module10):予測データ・「品名実績」を元に、出荷回数の順位に応じてAB(1～46号機)と" & _
        "Cバラ(59～68号機)の間で入替候補を作成します(段は問いません。51～58号機・98号機・70号機以上の拡張エリアは" & _
        "対象外)。" & vbCrLf & vbCrLf
...
Sub EnsureExclusionSettingsSheet()
    ...
    wsSet.Range("A1").Value = _
        ...
        "連番付けする)。ブロック外の号機(沼南ではCバラ01=61～73、Cバラ02=81～93、拡張X=それ以外)はAB編成の対象外" & _
        ...
        "(AB=1～46号機とCバラ=59～68号機の間で、出荷回数順に構成比92%・7%を目指します。51～58号機・98号機・" & _
        "70号機以上の拡張エリアは入替の対象外です。出荷回数のランキングに使う「品名実績」の曜日は" & _
