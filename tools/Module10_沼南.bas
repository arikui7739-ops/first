...
Sub CreateRelocationPlan()
    ...
    Do While outCnt < relocCount
        ...
        Do While srcRow > 0 And partnerRow = 0
            partnerRow = FindBestPartnerRow(dictByDan(CStr(rowDan(srcRow))), rowUsed, rowMach, CLng(bestOverMach), actualCountByMach, dictTargetRatio, targetRatioSum, targetedHitTotal, abBlockFrom, abBlockTo, abBlockCount, srcRow, dictRowMach, dictRowCat, dictRowWt, dictRowVol, dictMachCatCount, catWeight, sizeWeight, weightWeightCoef)
