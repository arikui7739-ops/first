...
Sub CreateRelocationPlan()
    ...
    Do While outCnt < relocCount
        ...
        Do While srcRow > 0 And partnerRow = 0
            partnerRow = FindBestPartnerRow(dictByDan(CStr(rowDan(srcRow))), rowUsed, rowMach, CLng(bestOverMach), actualCountByMach, dictTargetRatio, targetRatioSum, targetedHitTotal, abBlockFrom, abBlockTo, abBlockCount)
    ...
    wsOut.Cells(2, 1).Value = "「設定」シートの■号機別目標構成比に近づけるよう、目標超過号機の高頻度ロケーションと目標未達号機のロケーションを、同じ段の中で入れ替える指示です。"
    wsOut.Cells(2, 1).HorizontalAlignment = xlLeft

    wsOut.Range("A4:L4").Value = Array("段", "移動元号機", "移動元列", "移動元品コード", "移動元品名", "移動元予測回数", "移動先号機", "移動先列", "移動先品コード", "移動先品名", "移動先予測回数", "構成比(移動元/移動先:変更前→変更後)")
...
Private Function FindBestPartnerRow(rowsCol As Collection, rowUsed() As Boolean, rowMach() As Long, ByVal excludeMach As Long, actualCountByMach As Object, dictTargetRatio As Object, ByVal targetRatioSum As Double, ByVal targetedHitTotal As Double, abBlockFrom() As Long, abBlockTo() As Long, ByVal abBlockCount As Long) As Long
    Dim bestRow As Long: bestRow = 0
