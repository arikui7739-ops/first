Attribute VB_Name = "Module1_石狩"
Option Explicit

' ----------------------------------------------------
' 同時ピッキング改善指示(Cバラエリア: 51～58号機・61～68号機)
...
Sub SwapLocationsByCorrelationFast_Fix()
    ...
    If outCnt > 0 Then
        ...
        End If
        Call LogModule1KPI(totalWaste / outCnt, outCnt, reportDate)
...
' ロケーションキー(locKey)からCFシートの品名を引く。見つからない場合は「(品名不明)」を返す
...
' 「操作パネル」シートに同時ピッキング改善指示ボタンが無ければ追加する
...
Sub EnsureSwapCorrelationButton()
    ...
    On Error Resume Next
    Set existing = wsPanel.Shapes("同時ピッキング改善指示ボタン")
    On Error GoTo 0
    If existing Is Nothing Then
        Dim btn As Button
        Set btn = wsPanel.Buttons.Add(wsPanel.Range("H4").Left, wsPanel.Range("H4").Top, 220, 36)
        btn.Name = "同時ピッキング改善指示ボタン"
        btn.OnAction = "SwapLocationsByCorrelationFast_Fix"
        btn.Characters.Text = "同時ピッキング改善指示を作成"
        btn.Font.Size = 12
        btn.Font.Bold = True
    End If
